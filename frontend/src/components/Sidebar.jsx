...
export default function Sidebar({ isOpen, user }) {
  ...
  return (
    <aside 
      className={`
        fixed left-0 top-16 h-[calc(100vh-4rem)] bg-black w-64 z-40 shadow-2xl border-r border-gray-800
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-xs sm:text-sm font-bold text-white truncate">
              {user?.user_metadata?.first_name || user?.email?.split('@')[0] || 'Faculty Member'}
            </p>
